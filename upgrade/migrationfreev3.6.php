...
<div align=center style="padding-top:220px;">
	<form name="upgrade_to_latest" method="post">
		<label style="font-size:2em;" id="step1"><?php echo __('Upgrade to Latest Version 3.6','wp-advanced-importer'); ?></label>
		<input type="submit" class="btn btn-primary btn-sm" name="upgrade" id="upgrade" value="<?php echo __('Click Here','wp-advanced-importer'); ?>"/>
	</form>
	<form name="goto_plugin_page" method="post"
		  action="admin.php?page=<?php echo WP_CONST_ADVANCED_XML_IMP_SLUG; ?>/index.php&__module=settings">
		<label style="font-size:2em;display:none;" id='upgrade_state'><?php echo __('Upgrade is inprogress...','wp-advanced-importer'); ?></label>
		<input type="submit" style="display:none;" class="btn btn-success" name="gotopluginpage" id="gotopluginpage"
			   value="<?php echo __('Goto Plugin Settings','wp-advanced-importer'); ?>"/>
	</form>
</div>
<?php
if (isset($_POST['upgrade'])) {
	...
	global $wpdb;
		...
		$wpdb->query($sql1);
                ...
                $check_table2 = 'smackxml_line_log';
